...
\`fact\` using Keys cubic convolution interpolation, with a
pre-sharpening filter that makes the result mass-preserving: aggregating
it back by block mean recovers the input coarse values up to a small
truncation error governed by \`radius\`.
...
- coarse:
  ...
  SpatRaster. Multi-layer rasters are supported.
...
- radius:
  ...
  \`radius\` is set to \`max(9, fact + 4)\`, slightly larger than
  bilinear's default because cubic's inverse kernel decays more slowly.
...
- max_radius_frac:
  ...
  Numeric. Same semantics as in \[disagg_bl()\]; upper bound on kernel
  radius as a fraction of the coarse raster's smaller dimension.

- na_fill:

  Character. Boundary/NA handling mode, same options as in
...
- Cubic produces smoother output (no kinks at cell centers, where
  bilinear has them) at modestly higher computational cost.

- Cubic has negative side lobes and may overshoot input range near sharp
  gradients. For data with hard physical bounds (precipitation,
  fractional cover), this may matter.

- At the same \`radius\`, cubic's truncation error is somewhat larger
  than bilinear's because its inverse kernel decays more slowly. The
  default \`radius\` is correspondingly larger to compensate.

\*\*Mass preservation is approximate.\*\* Like \[disagg_bl()\], the
inverse kernel is a finite approximation to an ideal infinite operator.
The truncation error is concentrated near the raster boundary and decays
...
that \`terra::resample(method = "cubic")\` uses Keys cubic with \`a =
-0.5\`, which is the convention our pre-sharpening kernel inverts.
