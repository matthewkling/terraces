...
functions in other packages. But when your raster values conceptually
represent averages of finer-scale variation within each cell — perhaps
the most common case — these interpolation methods are inappropriate.
Their use for disaggregation implicitly assumes that raster values
represent point samples at grid cell centers; when used to disaggregate
mean-based rasters, they produce fine-scale results that are biased
toward the mean and cannot be re-aggregated to recover the original
coarse values.
